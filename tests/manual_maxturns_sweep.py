...
for max_turns in [3, 4, 5]:
    print(f"=== --max-turns {max_turns} ===")
    for i in range(4):
        proc = subprocess.run(
            ["claude", "-p", "-", "--output-format", "json", "--tools", "", "--max-turns", str(max_turns)],
            input=PROMPT, capture_output=True, text=True, timeout=120,
        )
        d = json.loads(proc.stdout)
        print(f"  attempt {i+1}: subtype={d.get('subtype')} num_turns={d.get('num_turns')} result={d.get('result')!r}")
